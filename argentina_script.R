...
## The incarceration rates grow from [1990-2002) and [2015-end). The incarceration 
## rates stop growing in [2002-2015) and the release rates
## stop decreasing in this time period. The release rates decrease in the other two
## intervals. The incarceration rates decrease and release rates
...
change.r.factor.1.input = .628 # halving release rate over entire time period
change.r.factor.2.input = 1
change.r.factor.3.input = .793
recid_perct_known = .28
recid_perct_year_known = 29
covid.start.input = 30 # 2020
covid.end.input = 32  # 2022
rel_one_years.input = 12
rel_two_years.input = 13
...
argentina_ip <- argentina_ip[complete.cases(argentina_ip), ] 
...
argentina_ad <- argentina_ad[complete.cases(argentina_ad), ] 
...
country_incarc_rates <-  get_iE_iR_grow(recid_perct_known = .28, recid_prob_known = 0, 
                                        recid_perct_year_known = 29, recid_prob_year_known = 0,
                                        ...
                                        muP =0.001058423, muS = 0.001058423, muR = 0.0170976, muE = 0.01628343,
                                        muN =0.01628343) # using 1/LE for muN and .065 and 1.05 for muR and muP ratios
...
country_incarc_rates <-list(iR = 0.05084869, iE= 0.0009285876, iN= 0.0009285876, r = 1.644962,
                            k = 8.119182e-05, k1 = 0 , k2 =1e-08,
                            covid_a= 1, covid_r = 1)
...
all_params <- c(iR =  country_incarc_rates$iR, iE= country_incarc_rates$iE, 
                ...
                k2 = country_incarc_rates$k2, covid_a=  country_incarc_rates$covid_a, 
...
## Note: k1 is set to 0 since
## we specified no growth in the second interval.
...
# Set true values
df2 <-data.frame(year =(as.numeric(argentina_ip$Year)), 
                 aip =as.numeric(argentina_ip$`Adjusted.Incarceration.Prevalence`))
...
g<-ggplot( df, aes(x=years_ip + 1990)) + 
  ...
  ggtitle(label = 'Argentina Incarceration Prevalence Over Time') +
...
g<-ggplot( df, aes(x=years_ad + 1990)) + 
  ...
  theme_bw() +
  ggtitle(label = 'Argentina Admissions Rates Over Time') +
  theme(axis.title.x = element_text(size = 15)) +
  theme(axis.title.y = element_text(size = 15)) +
  theme(axis.text.x = element_text(size = 11)) +
  theme(axis.text.y = element_text(size = 11)) +
