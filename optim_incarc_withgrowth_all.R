  ...
  
{
  ...
  if(country == "Brazil"){
    params[5] = params[4] # one growth interval
    params[6] = params[4] # one growth interval
  ...
  if(country == "Colombia"){
    ...
    #params[3] = 0.9595281
    #params[3] = 0.3267821
    params[6] =  params[5] # two growth intervals only
  }
  
  all_params <- c(iR = params[1], iE=params[1]*params[2], iN=params[1]*params[2], r=params[3], k=params[4],k1=params[5],k2=params[6],
                  covid_a=params[7], covid_r=params[8], l1 = params[9], l2 = params[10], l3 = params[11], #l1 = 0, l2 = 0, l3 = 0,
                  muP=unvarying_params[1], muR=unvarying_params[2], muE=unvarying_params[3], muS=unvarying_params[4],
                  muN=unvarying_params[5], a=unvarying_params[6]) # l1=params[9],l2=params[10], l3=params[11],
...
#scale <-c(0.25005000, 0.10005000, 1.10000000, 0.005000005, 0.005000005, 0.005000005, .55, .55,1.65,1.65,1.65) 
...
get_iE_iR_grow <- function(
  ...
  return(list(iR=iR, iE=iE, r=r, k=k,k1=k1, k2=k2, covid_a=covid_a,covid_r=covid_r,l1=l1,l2=l2,l3=l3, error =error)) #,l1=l1,l2=l2,l3=l3
}
...
country_incarc_rates <-  get_iE_iR_grow(recid_perct_known = .223, recid_prob_known = 0, recid_perct_year_known = 29,
                                        ...
                                        ip_known = ip_known, years_ad = years_ad, 
                                        ad_known=ad_known,start.incr = start.incr, stop.incr = start.incr + time.passed,
                                        param_start = c(0.05511821, 0.008507921, .5034239, 1e-06, 0, 0,0.6902856,1.345376,0,-0.03435735, 0.01664506),
                                        param_lower_bounds=c(0.0001, 0.0001, 0.2, 0.00000001, -0.00001, -0.00001,.1,1,0,-.056,0), 
                                        param_upper_bounds=c(.5,.2, 2, .01,0,0, 1,4,.14,0,.14),
                                        scale =c(0.25005000, 0.10005000, 1.10000000, 0.005000005, 0.005000005, 0.005000005, .55, .55,.07,.028,.07),
                                        muP = 0.0111943, 
                                        muS = 0.0111943,
                                        muR = 0.0180831, 
                                        muE = 0.017222,
